...
const accounts = [
  ...
  { code: "1060", name: "貸倒引当金",   type: "asset", category: "contra_asset",   sortOrder: 18 },
...
const insertAccount = db.prepare(`
  INSERT OR IGNORE INTO Account (code, name, type, category, description, isDefault, isActive, sortOrder, createdAt, updatedAt)
  VALUES (?, ?, ?, ?, NULL, 1, 1, ?, ?, ?)
`);

const insertMany = db.transaction(() => {
  for (const a of accounts) {
    insertAccount.run(a.code, a.name, a.type, a.category, a.sortOrder, now, now);
  }
});
insertMany();
console.log(`  勘定科目: ${accounts.length}件`);
...
console.log(`  設定: ${settings.length}件`);
...
const insertFiscalYear = db.prepare(`
  INSERT OR IGNORE INTO FiscalYear (year, startDate, endDate, isClosed, createdAt)
  VALUES (?, ?, ?, 0, ?)
`);
insertFiscalYear.run(year, startDate, endDate, now);
